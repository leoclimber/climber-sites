...
★ CINEMATIC MOMENT 1 ★ — SECTION 1: HERO WITH THE STACK VIDEO (id="hero")
   ...
   ★ KEEP THE BURGER BRIGHT, BIG & VISIBLE — BUT TEXT MUST BE PERFECTLY LEGIBLE: The video/food stays bright and vivid, BUT the headline and text must ALWAYS be effortlessly readable over it — legibility is non-negotiable on a premium site. Achieve BOTH with a DIRECTIONAL gradient scrim, not a flat wash: place a stronger dark scrim concentrated in the TEXT ZONE (lower-left, roughly the left 55% and lower portion where the eyebrow, headline, subtitle, CTAs and stats sit), fading to transparent toward the right/top so the burger stays bright and lit there. Concretely, layer a gradient like linear-gradient(105deg, rgba(10,8,6,0.82) 0%, rgba(10,8,6,0.55) 38%, rgba(10,8,6,0.15) 62%, transparent 82%) OVER the video (a dedicated overlay div, z-index between video and text), plus a subtle bottom-up gradient for the stats row. The text zone should be dark enough that cream text reads with strong contrast even where cheese/ketchup/highlights are bright behind it; the right side of the frame stays bright and appetising. Additionally give the headline a soft text-shadow (e.g. 0 2px 24px rgba(0,0,0,0.55)) as a safety net so it never gets lost against bright cheese or splashes. Do NOT wash the WHOLE video in heavy darkness (that kills the appetite) and do NOT let the food read small — the food must POP on the right/upper area. No border, no box, no rounded frame — the video bleeds to all four edges.
...
SECTION 1: HERO (id="hero") — PHOTO, NOT VIDEO
   ...
   ★★★ HERO TEXT PLACEMENT — STRICT (CRITICAL, prevents the text landing on top of the cup): The hero product image has the CUP roughly CENTERED/right. Therefore ALL hero text MUST live in a LEFT COLUMN, vertically centered on the left side, constrained to max-width ~46% of the viewport (max ~560px), with its left edge flush to the page's left margin/gutter (same as the nav logo, ~5-6vw). The text column MUST NOT extend past ~48% of the viewport width and MUST NOT overlap the cup in the center — keep a clear gap. Think two zones: LEFT = all text; CENTER/RIGHT = the cup, bright and unobstructed. A DIRECTIONAL scrim darkens ONLY the left text zone (e.g. linear-gradient(90deg, rgba(20,14,10,0.82) 0%, rgba(20,14,10,0.5) 34%, transparent 55%)) so text is perfectly legible while the cup stays bright; plus a soft text-shadow on the headline. Stack in the left column: eyebrow (a neighbourhood/city tag or "// FRESHLY BREWED"), a warm inviting headline, a short subtitle, two CTAs (primary "VIEW THE MENU" → #services, secondary "FIND US" → #contact), and a row of animated stat counters (${rating}★ · ${reviewCount} reviews · e.g. "OPEN FROM 7AM"). Everything left-aligned, sharing one clean vertical line with the logo. NEVER center the hero text, NEVER let it sit on top of the cup. Big brand wordmark in the nav.
...
★ CINEMATIC MOMENT 1 ★ — SECTION 1: HERO WITH THE POUR VIDEO (id="hero")
   ...
   ★★★ HERO TEXT PLACEMENT — STRICT: keep ALL hero text in a LEFT COLUMN (max-width ~46% viewport, ~560px), left edge flush to the page margin (~5-6vw), vertically centered on the left. It MUST NOT extend past ~48% width and MUST NOT overlap the cup/pour in the center of the video — LEFT = text, CENTER/RIGHT = the coffee, bright and unobstructed. The directional scrim darkens ONLY the left text zone; headline gets a soft text-shadow. NEVER center the hero text, NEVER let it sit on top of the cup.
