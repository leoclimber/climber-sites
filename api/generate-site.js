...
★ CINEMATIC MOMENT 1 ★ — SECTION 1: HERO WITH THE STACK VIDEO (id="hero")
   ...
   Text placement (CRITICAL — do not cover the burger/video): eyebrow ("// STONEYBATTER · DUBLIN"), a LARGE, bold, imposing headline (make it BIG and confident — in the last version it was too small/timid; the headline should command the screen like a premium product reveal, large condensed display type, multiple lines), a short subtitle, two CTAs (primary "CALL TO ORDER" → ${bookingHref}, secondary "SEE THE MENU" → #services), and a row of animated stat counters (${rating}★ rating · ${reviewCount} reviews · 100% fresh) — all anchored to the LEFT side / lower-left, over a soft gradient scrim, with the video/burger occupying the right and center. Big headline, but positioned so it does not sit on top of the burger. The food is the hero; the type is large and supports it. A tiny "SCROLL TO BEGIN" cue at the bottom. Big brand wordmark in the nav.
