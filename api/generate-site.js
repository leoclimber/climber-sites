...
SECTION 1: HERO (id="hero") — PHOTO, NOT VIDEO
   Full-bleed hero using the hero coffee image ${photoList[0]} as a big, close, inviting background (object-fit:cover, filling the viewport) — steam, latte art, warm light. ${hasVideo ? `(A client hero video was provided — you MAY use it as a calm full-bleed background with a light scrim; keep it gentle, no elaborate treatment.)` : `No video — the big warm photo IS the hero.`} A soft directional scrim concentrated in the TEXT ZONE (lower-left) so text is perfectly legible while the coffee stays warm and bright elsewhere; give the headline a soft text-shadow as a safety net. Over it: eyebrow (a neighbourhood/city tag or "// FRESHLY BREWED"), a warm inviting headline, a short subtitle, two CTAs (primary "VIEW THE MENU" → #services, secondary "FIND US" → #contact), and a row of animated stat counters (${rating}★ · ${reviewCount} reviews · e.g. "OPEN FROM 7AM"). All left-aligned and ANCHORED TO THE LEFT MARGIN of the page (same left gutter as the nav logo, ~5-6vw), sharing one clean vertical line with the logo — not floating in the middle. Big brand wordmark in the nav.
