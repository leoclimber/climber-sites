...
★ CINEMATIC MOMENT 1 ★ — SECTION 1: HERO WITH THE STACK VIDEO (id="hero")
   ★ FULL-BLEED HERO VIDEO (like the docmo reference — this is the signature first impression): The STACK VIDEO fills the ENTIRE hero viewport as a full-screen background. ${bankVideos.stack ? `Use the video "${bankVideos.stack}" as a TRUE FULL-BLEED background: <video autoplay muted loop playsinline style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover;z-index:0;"> covering the whole hero edge-to-edge, the exploding burger large and visible filling the screen. Add MANDATORY onerror AND onstalled fallback that swaps the video src to the hero burger image ${photoList[0]} (NEVER a fries image), also full-bleed, so the hero always shows a big burger.` : `Use the hero burger image ${photoList[0]} as a full-bleed background.`}
   ★ KEEP THE BURGER BRIGHT & VISIBLE: Use only a MINIMAL dark overlay — just enough to keep text legible, and concentrate it BEHIND the text area (e.g. a left-to-right or bottom gradient scrim that darkens only the text zone), leaving the burger itself bright, lit and clearly visible. Do NOT wash the whole video in heavy darkness — the food must pop. No border, no box, no rounded frame — the video bleeds to all four edges.
   TEXT OVER THE VIDEO: eyebrow ("// STONEYBATTER · DUBLIN"), a LARGE bold imposing headline (big condensed display, multiple lines, commanding), a short subtitle, two CTAs (primary "CALL TO ORDER" → ${bookingHref}, secondary "SEE THE MENU" → #services), and a row of animated stat counters (${rating}★ · ${reviewCount} reviews · 100% fresh) — anchored lower-left over the gradient scrim, z-index above the video. A tiny "SCROLL TO BEGIN" cue at the bottom. Big brand wordmark in the nav. The video is the immersive backdrop; the text sits cleanly on top of its darker (left/bottom) zone while the burger stays bright on the right/center.
...
★ CINEMATIC MOMENT 2 ★ — SECTION 3: // THE CUT (id="cut")
   The "anatomy" moment — clean and elegant, NO scroll-driven deconstruction (that broke and looked bad; do NOT animate the burger falling apart). Mono eyebrow "// THE CUT". LEFT: a big static "230°C" callout in ember + a spec table (Patty/Grind, Sear/Surface · 230°C, Cheese, Bun, Origin) in mono type + one line of copy. RIGHT: the WHOLE hero burger ${photoList[0]} shown intact and beautiful (a single clean image, NOT split into pieces), with thin elegant label lines pointing FROM the burger OUT to ingredient labels around it (SESAME BRIOCHE, DOUBLE SMASH, AGED CHEDDAR, BEEFSTEAK TOMATO, BUTTER LETTUCE, BRIOCHE BASE) — like an anatomy/exploded-diagram callout but with the burger STAYING WHOLE. The labels can fade in gently on scroll (simple opacity reveal, staggered), but the burger image itself stays intact and still — do NOT slice it, do NOT drift the layers apart. Clean, premium, lots of dark space around it.
